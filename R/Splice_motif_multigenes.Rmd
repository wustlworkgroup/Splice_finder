...
```{r}
Path="/media/yang/Data/SliceFinder/out_predict"

```

#  The splice donor site is located at the very beginning of an intron (-5 IS THE END OF EXON IN DONOR SEQ)
## All splice site from multiple genes ()
```{r, include=F}
Donor_seq_files=list.files(Path, pattern ="dna_donor_seq",recursive = T,full.names = T )
Seqlist=list()
for (i in 1:length(Donor_seq_files)) {
  fasta_file=Donor_seq_files[i]
  
sequences <- Biostrings::readDNAStringSet(fasta_file)
Seqlist[[i]] <- as.character(sequences)
}
# Convert sequences to character vectors
print (paste0("gene number:", length(Donor_seq_files)))
seq_list=unlist(Seqlist)
...
## All acceptor site
```{r, include=F}
Acceptor_seq_files=list.files(Path, pattern ="dna_acceptor_seq",recursive = T,full.names = T )
Seqlist=list()
for (i in 1:length(Acceptor_seq_files)) {
  fasta_file=Acceptor_seq_files[i]
  sequences <- Biostrings::readDNAStringSet(fasta_file)
  Seqlist[[i]] <- as.character(sequences)
}
seq_list=unlist(Seqlist)
...
pfm <- generate_PFM(seq_list)
pwm <- seqLogo:: makePWM(pfm)
print (paste0("gene number:", length(Acceptor_seq_files)))
# Plot sequence logo
